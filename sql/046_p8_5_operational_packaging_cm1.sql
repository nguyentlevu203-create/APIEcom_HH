-- =====================================================================
-- 046_p8_5_operational_packaging_cm1.sql
-- P8.5 Sections 2-4 (approved) — wire hh_operational_packaging_cost into
-- CM1, additively, alongside the existing physical packaging_item_cogs
-- (unchanged, still the 'packaging_cost' Gold metric / hh_internal_
-- packaging_cost mart column, kept for backward compatibility).
--
-- Incremental CM1 change is ONLY the new hh_operational_packaging_cost
-- term -- it is NOT added on top of packaging_item_cogs a second time.
-- g.cm1 (from Gold) already has packaging_item_cogs subtracted by the
-- PNL_ENRICHMENT scripts; this view subtracts hh_operational_packaging_
-- cost ONCE, here, on top of that -- proven algebraically in-session:
-- CM1_new = CM1_old - hh_operational_packaging_cost, nothing else moves.
--
-- Respects the existing TikTok settlement-completeness guard (044) --
-- SOURCE_LAGGING/NOT_SETTLED_YET/NO_ELIGIBLE_ORDERS dates still show
-- cm1 = NULL, never a number built from an incomplete cohort.
-- =====================================================================
...
SELECT g.business_date,
    ...
    g.ads_spend,
    -- backward-compat name (unchanged meaning: physical packaging-item COGS)
    ...
    -- CM1: incremental change is ONLY hh_operational_packaging_cost -- g.cm1_raw
    -- already has packaging_item_cogs subtracted by the PNL_ENRICHMENT scripts.
    CASE
    ...
    -- P8.5 NEW columns, appended at the end (Postgres CREATE OR REPLACE VIEW
    -- cannot reorder/insert columns mid-list without a DROP).
